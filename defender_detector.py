...
def capture_ram_key():
    """
    [RAM FORENSICS INTERCEPTION]
    Process ကို Kill မပြုလုပ်မီ / Ransomware က AES Key မဖျက်ဆီးမီ 
    Volatile RAM Memory Buffer ထဲမှ RAW AES Key ကို ကြားဖြတ် ဖမ်းယူခြင်း
    """
    ...
    try:
        if os.path.exists(TEMP_KEY_FILE):
            ...
            if intercepted_key:
                print(f"[ DEFENDER FORENSICS] SUCCESS: RAM AES Key Intercepted! Key: {intercepted_key}")
...
def quarantine_malware():
    """Ransomware Payload Script/File ကို Quarantine Vault ထဲသို့ အလိုအလျောက် ရွှေ့ပေးသည့် Function"""
    try:
...
def generate_forensic_report(killed_pid, intercepted_key):
    report_data = {
        ...
        "intercepted_aes_key": intercepted_key if intercepted_key else "NOT_CAPTURED",
        "action_taken": (
